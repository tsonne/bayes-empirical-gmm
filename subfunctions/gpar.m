...
%        n _ nrow(x)
    %x=r((n1/2+1):n1,:); % original
    x=r(floor(n1/2+1):n1,:); % MODIFIED BY TSONNE 2015-02-22
    n=max(size(x));
...
%        quantiles _ quantile (as.vector(x), probs=c(.025,.25,.5,.75,.975))
        % quantiles=prctile(x,100*[0.025,0.25,0.5,0.75,0.975]);  % TS20150810    
...
%        postdf=chisqdf(postvar,varpostvar);
   ...
   % postrange=muhat+sqrt(postvar)*tinv(1-alpha/2,postdf)*[-1,0,1];  %TS
...
elseif (max(size(size(r)))==3)
   ...
   % Compute the acceptance ratio.
    ...
    for jj=1:m
        xdot(jj)=mean(x(1-isnan(x(:,jj)),jj));   %TS
        s2(jj)=var(x(1-isnan(x(:,jj)),jj));   %TS
    end
...
%    quantiles=quantile(x,[0.025,0.25,0.5,0.75,0.975]);     
    ...
    if (W > 1.e-8)   % non-degenerate case % TS: neff requires postvar
